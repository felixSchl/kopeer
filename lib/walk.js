var
    Promise = require('bluebird')
  , fs      = Promise.promisifyAll(require('fs'))
  , path    = require('path')
  , _       = require('lodash')
;

module.exports.dir = function(top, options) {

    options = _.defaults(options || {}, {
          filter:      _.constant(true)
        , followLinks: false
    });

    return _walkDirectory(top, top, options.filter, options.followLinks);
}

function _walkDirectory(top, dir, filter, followLinks) {
    return fs.readdirAsync(dir)
        .map(function(filename) {
            return Promise.resolve(
                path.resolve(dir, filename)
            );
        })
        .filter(function(fullpath) {
            return Promise.resolve(
                filter
                    ? filter(path.relative(top, fullpath))
                    : true
            )
        })
        .map(function(filepath) {
            return (fs[followLinks ? 'statAsync' : 'lstatAsync'])(filepath)
                .then(function(stat) {
                    return Promise.resolve({
                          filepath: filepath
                        , relpath:  path.normalize(path.relative(top, filepath))
                        , stats:    stat
                    })
                })
            ;
        })
        .map(function(entry) {
            if (entry.stats.isDirectory()) {
                return _walkDirectory(top, entry.filepath, filter, followLinks);
            }
            else {
                return entry;
            }
        })
        .reduce(function(a, b) {
            return a.concat(b);
        }, [])
    ;
}

